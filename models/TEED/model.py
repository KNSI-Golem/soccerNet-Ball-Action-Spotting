...
#Local imports
from models.TEED.modules import BaseRGBModel, EDSGPMIXERLayers, FCLayers, FC2Layers, step, process_prediction, process_predictionTeam, process_double_head
from models.TEED.shift import make_temporal_shift
...
class TDEEDModel(BaseRGBModel):
    ...
    class Impl(nn.Module):
        ...
        def update_pred_head(self, num_classes = [1, 1]):
            ...
            self._pred_fine = self._pred_fine.cpu()
    ...
    def __init__(self, device=None, args=None):
        ...
        self.device = device if device else torch.device(self.device_type)
    ...
    def predict(self, seq, use_amp=True):
        ...
        self._model.eval()
        with torch.no_grad():
            with torch.amp.autocast(self.device_type) if use_amp else nullcontext():
